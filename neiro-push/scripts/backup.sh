#!/usr/bin/env bash
# Бэкап neiro-push: база SQLite + .env с Pi на локальную машину.
# Запускать ПЕРЕД любым деплоем со схемой/миграциями.
#
#   ./neiro-push/scripts/backup.sh          # снять бэкап
#   ./neiro-push/scripts/backup.sh --list    # показать, что уже снято
set -euo pipefail
...
ls -1t ~/"${REMOTE_DIR}"/neiro_push-*.db 2>/dev/null | tail -n +11 | xargs -r rm -f
ls -1t ~/"${REMOTE_DIR}"/env-*.txt 2>/dev/null | tail -n +11 | xargs -r rm -f

echo "на Pi: ~/${REMOTE_DIR}/neiro_push-${STAMP}.db"
REMOTE

mkdir -p "${LOCAL_DIR}"
scp -q "${SSH_HOST}:~/${REMOTE_DIR}/neiro_push-${STAMP}.db" "${LOCAL_DIR}/"
scp -q "${SSH_HOST}:~/${REMOTE_DIR}/env-${STAMP}.txt" "${LOCAL_DIR}/"
chmod 600 "${LOCAL_DIR}/env-${STAMP}.txt"

echo ""
echo "Готово:"
echo "  ${LOCAL_DIR}/neiro_push-${STAMP}.db"
echo "  ${LOCAL_DIR}/env-${STAMP}.txt   (секреты, chmod 600, в git не попадает)"
echo ""
echo "Откат базы:  ./neiro-push/scripts/restore.sh neiro_push-${STAMP}.db"
