#!/usr/bin/env bash
# Reverse SSH-туннель Pi → VPS для neiro-push (публичный маршрут /v1).
# Запускается как systemd --user юнит neiro-push-tunnel.service,
# см. install-tunnel.sh.
set -euo pipefail

ENV_FILE="${HOME}/.config/neiro-push/tunnel.env"
if [[ -f "${ENV_FILE}" ]]; then
