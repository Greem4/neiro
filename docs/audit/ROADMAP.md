...
Документ — продолжение пакета `docs/refactoring/` (этапы 1–6). Сначала — сверка старого плана, затем новые находки, сгруппированные в волны A–E по приоритету.

## Правила работы (те же, что в docs/refactoring/)
...
- **Не трогать ничего из `docs/refactoring/OUT_OF_SCOPE.md`** — архив, `LaunchedEffect(initialNames)` в DayDetailsDialog, authoritative wipe текущего месяца и т.д.
- Один пункт = один коммит. Заготовки сообщений даны в конце каждого пункта.

---

## Статус старого плана (docs/refactoring/README.md)

Проверено по коду: **ETAP_1, 2, 3, 5, 6 фактически выполнены**, ETAP_4 выполнен почти весь. Чекбоксы acceptance criteria в `docs/refactoring/README.md` устарели — в коде уже сделаны:
