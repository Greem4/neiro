...
обновляется по таймеру. Освежение — при `onStart` и при следующем push'е. Это
решение пользователя, обратно не возвращать
([push-events-plan.md §12.2](push-events-plan.md)).
...
override fun onMessageReceived(message: RemoteMessage) {
    when (message.data["action"]) {
        "session_events" -> { /* разбор events → PushEventNotifier.notify → сохранить курсор */ }
        "sync_events" -> PushEventsSyncCoordinator.enqueue(applicationContext)
        "sync" -> PushSyncCoordinator.enqueue(applicationContext)   // старый сервер
    }
}
```

Ветку `"sync"` оставить: она стоит три строки, а на переходный период даёт
работоспособность, если телефон окажется зарегистрирован на старом сервисе.
...
### 5.2 `push/PushEventsSyncer.kt` (новый)
...
4. **Курсор и переустановка.** После переустановки приложения курсор `0`, а
   `device_id` новый — сервер отдаст всю ленту за 30 дней. Ограничение
   `limit` + фильтр горизонта спасают частично; если окажется, что при первом
   запуске сыпется десяток уведомлений — договориться с сервером о том, что новое
   устройство регистрируется с курсором на текущий максимум.

---

## 9. Приёмка приложения
...
Требуют решения пользователя до начала этапа B:

1. **Первый запуск после установки.** Показывать догоном всю ленту за 30 дней или
   регистрировать новое устройство с курсором на текущий максимум (риск 4)?
   Второе тише, но при переустановке потеряются события последних минут.

2. **Ветка `action = "sync"`.** Оставлена в §4.3 как страховка. Если решено, что
   новая сборка не должна уметь ходить на старый сервис вообще — вместе с ней
   удаляются `PushSyncWorker` и `PushSyncCoordinator`.
