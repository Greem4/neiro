...
Оплата — отдельное поле `paid_full` (`1` — оплачено); в интерфейсе YClients это
плюсик на записи, тогда как `attendance = 1` рисуется восклицательным знаком.
Приход оплатой не является: 01.09.2026 в дне было четыре записи с
`attendance = 1`, из них оплачены три. Приложение с этого дня различает их
статусами `ARRIVED` и `PAID`: «проведено» считается по приходу, деньги — по
оплате (`SessionParser.AttendanceStatus`). Отменённую запись оплата не
воскрешает — так выглядит списание абонемента за пропуск.
