...
Отсюда четыре жёстких требования к payload сервера. Ни одно из них в плане явно
не записано — если сервер сделает иначе, дубли поедут молча.
...
### 2.5 `staff_id` — событие принадлежит специалисту, а не устройству

**Критерий адресации — `staff_id`.** Это тот же идентификатор, что приходит в
`record.staff_id` из YClients и что устройство отправляет при регистрации
([PushRegistrar.kt:107](../../app/src/main/java/ru/greemlab/neiro/push/PushRegistrar.kt#L107)).
Он есть на обоих концах, поэтому сверять принадлежность события можно без
единого дополнительного поля в приложении.

**Требование к серверу: каждое событие обязано нести `staff_id` — и в payload
push'а, и в ответе догона. Требование к приложению: событие, чей `staff_id` не
совпадает со «своим», отбрасывается молча, до всех остальных фильтров.**

Формально устройство и так получает только события своего аккаунта — `devices`
привязаны к `accounts`, а аккаунт это пара `(company_id, staff_id)`. Но эта
привязка живёт **только на сервере**, и приложение никак не может её проверить,
пока `staff_id` не приезжает в самом событии.
...
**Чем это кончится без фильтра на устройстве.** Специалист получает уведомления
про чужих детей — «Иванов Ваня подтвердился» на занятие, которое ведёт коллега.
И хуже: `PushEventCalendarApplier` (§5) впишет чужую запись **в его календарь**,
где она останется до следующего полного синка. То есть пострадает не только
шторка уведомлений, но и данные на экране.

Своё значение приложение берёт из `repository.staffId` — то же поле, что уходит
в `RegisterDeviceRequest.staffId`
...
календарном applier'е по отдельности, а сразу при разборе payload, **до** того
как события разойдутся по двум путям:

```
разбор payload / ответа догона
        │
        ├── отсев чужого staff_id   ← здесь, один раз
        │
        ├──> PushEventNotifier      (§4.2)
        └──> PushEventCalendarApplier (§5)
```

Так невозможно забыть фильтр в одной из веток. Отсев — молчаливый, но с
`Log.w`: чужое событие на устройстве означает, что на сервере что-то поехало, и
это должно быть видно в логах, а не просто исчезать.

Фильтр применяется **к обоим путям доставки** — и к push'у, и к догону (§6):
сервер в обоих случаях один и тот же, значит и ошибиться может в обоих.

Это защита в глубину: серверная раскладка по `staff_id` остаётся основной и
никуда не девается. Фильтр на устройстве — второй рубеж на случай, если первый
молча сломается.
...
data class PushSessionEvent(
    val id: Long,
    val staffId: Int,      // кому адресовано событие, см. §2.5
    val type: String,
...
**`staffId` обязателен** (уточнено 26.07.2026). Без него фильтр из [§2.5](#25-staff_id--событие-принадлежит-специалисту-а-не-устройству)
писать не из чего, а он — второй рубеж защиты от того, что серверная раскладка
по специалистам однажды сломается. Поле приходит и в push'е, и в ответе догона
([план §6.1](plan.md)), в обоих случаях в snake_case — `@SerializedName("staff_id")`.

Отсев — сразу после разбора, **до** того как события разойдутся по нотификатору
и календарному applier'у:

```kotlin
val mine = parsed.filter { event ->
    val ok = event.staffId == repository.staffId
    if (!ok) Log.w(TAG, "чужое событие: staff=${event.staffId}, своё=${repository.staffId}")
    ok
}
```

Своё значение — `repository.staffId`
([YClientsRepository.kt:48](../../app/src/main/java/ru/greemlab/neiro/data/network/YClientsRepository.kt#L48)),
то же самое, что уходит при регистрации
([PushRegistrar.kt:116](../../app/src/main/java/ru/greemlab/neiro/push/PushRegistrar.kt#L116)).
Оно всегда заполнено у залогиненного пользователя: без него регистрация не
проходит вовсе ([PushRegistrar.kt:107](../../app/src/main/java/ru/greemlab/neiro/push/PushRegistrar.kt#L107)).
Если `repository.staffId` всё же `null` — события **не** показывать: своего
идентификатора нет, сверить не с чем.
...
при разборе payload (§2.5), одним барьером на оба пути. Если по какой-то причине
барьер решено ставить не там, он всё равно обязан быть **до**
`PushEventCalendarApplier`, а не только перед показом уведомлений: иначе чужая
запись попадёт в календарь, даже не показавшись в шторке.
...
**`ack` обязателен** (подтверждено пользователем 26.07.2026). Ранее здесь стояла
пометка «необязательно, только для дашборда» — она устарела ещё 25.07.2026,
когда приняли решение о курсоре из регистрации (§6.4). Без `ack` серверный
курсор не двигается, и после переустановки приложения устройство попадёт в ветку
«известный `device_id` — курсор не трогаем» со **старой** позицией: догон
вывалит всю ленту, которую это решение и должно было отсечь. `device_id` живёт в
`ANDROID_ID` и переустановку переживает, так что случай не гипотетический.

Слать после каждой успешно показанной пачки, вместе с сохранением курсора в
prefs — те же события, тот же момент.
...
4. Каждую страницу: разбор → отсев чужого `staff_id` ([§2.5](#25-staff_id--событие-принадлежит-специалисту-а-не-устройству))
   → **применение к календарю** (§5) → `PushEventNotifier.notify(...)`.
   Порядок и обработка ошибок — те же, что в [§5.7](#57-порядок-календарь-потом-уведомление--но-независимо).
...
7. События применять **в порядке возрастания `id`** — иначе «перенос, затем
   отмена» ляжет наоборот (риск 6 в §9).

**Почему догон тоже правит календарь** (уточнено 26.07.2026). В §5.1 это уже
сказано («вызывается из FCM-сервиса и из догона»), но в шагах выше отсутствовало
— дописано, чтобы не разошлось.

Строго говоря, в большинстве случаев без этого можно обойтись: догон при открытии
приложения вызывается рядом с `refreshNow`, а тот тянет из YClients текущий и
следующий месяц целиком — календарь и так станет актуальным, причём данными
надёжнее payload'а.

Дыра — в нудже (`action = "sync_events"`, §6.3): сервер говорит «событий много,
забери сам», приложение при этом открыто, синка рядом нет. Уведомления
покажутся, а календарь останется прежним — и после этапа 8.1, когда пятиминутный
опрос уберут, останется таким до следующего открытия приложения.

Случай узкий (нудж срабатывает, только когда payload перевалил за 3 КБ), но
применение стоит ноль — события уже разобраны и лежат в руках. Заодно оба пути
доставки ведут себя одинаково, а расхождения между путями нам в этом проекте уже
обходились дорого.
...
- `POST /v1/devices/register` для **нового** `device_id` ставит
  `last_ack_event_id = max(events.id)` — максимум по **всему журналу**, без
  фильтра по аккаунту (уточнено 26.07.2026, см. ниже).
...
**Почему максимум по всему журналу, а не по аккаунту** (уточнение 26.07.2026).
`events.id` сквозной для всех аккаунтов, поэтому «конец журнала» — одно число
для всех. Максимум по аккаунту всегда меньше или равен общему, то есть ставит
закладку в прошлое относительно журнала: при первом же запросе устройство
попросит события, которые ему не адресованы, и от чужих его отделит только
серверный фильтр по `account_id`. Выгоды никакой, лишний рубеж риска — есть.

Смысл решения от этого не меняется: человек с новым телефоном при входе и так
получает полный синк календаря — текущий и следующий месяц целиком. Уведомления
о том, что случилось до регистрации, ему не нужны; актуальное состояние уже на
экране.

**Следствие: `ack` обязателен.** С этим решением курсор обязан жить и на
сервере, иначе после переустановки серверный курсор окажется на старой позиции и
лента всё-таки польётся. Слать `ack` после каждой успешно показанной пачки —
сигнатура и обоснование в [§6.1](#61-pushapikt).
...
Читается в [app/build.gradle.kts:66](../../app/build.gradle.kts#L66). До сервиса
доходят обычные `/v1/...` — префикс `/v2` снимается по дороге.

**Кто именно его снимает — nginx на VPS, а не Caddy** (проверено на живых
серверах 26.07.2026). Реальный маршрут:

```
https://push.neiro.greemlab.ru
        ↓
VPS 176.12.65.86 — nginx + сертификат Let's Encrypt
        ├─ location /v2/  →  127.0.0.1:18082
        └─ location /     →  127.0.0.1:18080
                ↓  обратные SSH-туннели, поднимает малина
малина
        ├─ 18082 → 127.0.0.1:8011   neiro-push-events
        └─ 18080 → 127.0.0.1:8010   neiro-push (старый)
```

Префикс срезает **слеш в конце `proxy_pass`** — `proxy_pass
http://127.0.0.1:18082/;` в `/etc/nginx/sites-available/push.neiro.greemlab.ru`
на VPS. Уберут слеш — все пути поедут на `/v2/v1/...`, а причину будут искать в
Caddy, которого в этом маршруте нет.

Caddy на малине **в пути с улицы не участвует.** Блок
`http://push.neiro.greemlab.ru { reverse_proxy neiro-push:8010 }` в его конфиге
есть, но туннель приходит прямо на порт контейнера, мимо него. Подробности, как
...
- [ ] Событие с чужим `staff_id` → уведомления нет, календарь не тронут, в логах
      `Log.w` (§2.5). Проверять подменой поля в payload, не дожидаясь настоящей
      поломки сервера
- [ ] Два телефона одного специалиста → одно и то же событие пришло на оба
