...
8. ~~**Собрать и поставить приложение 0.7.0.0**~~ — сделано 27.07.2026.
   ...
   **Осталось проверить живое уведомление** — сделать реальное изменение в
   YClients (подтвердить/отменить/перенести занятие) и убедиться, что событие
   появилось в ленте дашборда с доставкой `1/1` и пришло на телефон.
9. **Почистить `deploy.sh`** от мёртвого шага с Caddy (см.
   [подводный камень 9](#9-deploysh-патчит-caddy-который-по-факту-не-участвует-в-маршруте))
   — отдельная задача.
10. **Этап 10** — гашение старого `neiro-push`. Не раньше недели стабильной
