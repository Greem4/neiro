...
локальной ветке `фикс-уведомлений`. Дальше — деплой (см. «Что дальше» ниже).
...
6. ~~**Этап 9** — документация по факту~~ — сделано: README сервиса,
   [docs/push-events.md](../push-events.md), [docs/push-setup.md](../push-setup.md).
7. **Деплой на Pi** — локальные коммиты этапов 1–9 ещё не выкачены.
   **Следующая работа.** Не забыть `DELETE FROM record_states;` перед первым
   запуском сервера. Переход на новый push обратной совместимости со старым
   сервисом не имеет — устройства переводятся по одному, вместе с обновлением
   приложения до 0.7.0.0.
8. **Этап 10** — гашение старого `neiro-push`. Не раньше недели стабильной
