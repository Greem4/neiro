...
| — | Деплой на Pi: `rsync` + `docker compose up -d --build`, без правки Caddy. Проверено: `/health` (локально и `/v2` публично) отвечает `200`, `fcm_configured: true` | `scripts/deploy-no-caddy.sh` (не в git) |
