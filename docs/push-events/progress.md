...
10. **Почистить `deploy.sh`** от мёртвого шага с Caddy (см.
    ...
    — отдельная задача.
11. **Этап 10** — гашение старого `neiro-push`. Не раньше недели стабильной
    работы новой связки после деплоя из пункта 7.

    **Решение от 30.07.2026:** до выключения код `server/` не правим вообще —
    он доживает как есть и гасится, когда все устройства перейдут на
    `neiro-push-events`. Находки аудита по нему (`K2`–`K4`, `K6`) сняты с
    дорожной карты, см. [`docs/audit-30.07.26/ROADMAP.md`](../audit-30.07.26/ROADMAP.md).
