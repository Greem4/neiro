...
1. Открой [plan.md](plan.md), раздел **«Этап 7»** (дашборд, `§8.4`) — это
   следующая нетронутая работа.
2. Работай этапами из [CLAUDE.md](../../CLAUDE.md): один коммит — одна
   ...
   этапам».
3. Когда Этап 7 закрыт — переходи к Этапу 8 ([app.md](app.md), уже написан и
   ждёт правок приложения).
...
Этапы 0–6 **закрыты полностью**, включая обе сверки с планом по 1–5:
...
локальной ветке `фикс-уведомлений`. Дальше — Этап 7 (дашборд), см. блок выше.
...
3. ~~**Этап 6** — API регистрации и догона~~ — сделано, коммит ниже.
4. **Этап 7** — дашборд (§8.4 плана). **Следующая работа, план в блоке
   «Начинаешь в новом окне?» выше.**
