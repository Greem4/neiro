...
1. Открой [plan.md](plan.md), раздел **«Этап 6»** (API регистрации устройства и
   догона, `§6.2`) — это следующая нетронутая работа.
2. Учти два примечания, ссылки на которые не потеряй:
   [про курсор за пределами ретеншена](stage5-review.md#5-наблюдения-на-будущее-не-трогать-сейчас)
   и то, что богатый `/health` (`Database.stats()`) уже написан и относится
   сюда же.
3. Работай этапами из [CLAUDE.md](../../CLAUDE.md): один коммит — одна
   ...
   этапам».
4. Когда Этап 6 закрыт — переходи к Этапу 7 (дашборд, §8.4 плана), затем к
   Этапу 8 ([app.md](app.md), уже написан и ждёт правок приложения).
...
Этапы 0–5 **закрыты полностью**, включая обе сверки с планом:
...
**Ничего из этого ещё не задеплоено на Pi** — все 10 коммитов только в
локальной ветке `фикс-уведомлений`. Дальше — Этап 6, см. блок выше.
...
3. **Этап 6** — API регистрации и догона (§6.2 плана). **Следующая работа,
   план в блоке «Начинаешь в новом окне?» выше.** Учесть
   [замечание про курсор за пределами ретеншена](stage5-review.md#5-наблюдения-на-будущее-не-трогать-сейчас)
   и то, что богатый `/health` из §6.2 — тоже этот этап, `Database.stats()` под
   него уже написан.
4. **Этап 7** — дашборд (§8.4 плана).
5. **Этап 8** — правки приложения, [app.md](app.md) — уже написан целиком.
6. **Этап 9** — документация по факту сделанного.
7. **Деплой на Pi** — все 10 локальных коммитов (правки этапов 1–4 и 5) ещё
   не выкачены. Не забыть `DELETE FROM record_states;` перед первым запуском.
