...
### Что попадает в журнал циклов, а что нет

Опрос идёт раз в 10 секунд, то есть примерно **8300 циклов в сутки** на
компанию. Изменений в них почти никогда нет: типичный цикл — «сходил, ничего
не поменялось». Записывать такой цикл незачем, и `poll_runs` его больше не
получает — решение принимает [`PollService._finish_run`](../../neiro-push/app/poller.py).

Разделение простое: **пульс — не история**.

| Что | Где живёт | Почему так |
|---|---|---|
| «Сервис жив, последний опрос был тогда-то, занял столько-то» | Память процесса: `PollService.last_run_at`, `last_run_duration_ms`, `last_run_error` | Нужно только последнее значение. После рестарта пусто не дольше одного интервала опроса, шапка дашборда показывает прочерк |
| События, пуши, ошибки циклов | Таблица `poll_runs`, 90 дней | Это то, к чему возвращаются при разборе «почему не пришло уведомление» |
| Пустой цикл | Нигде | 8300 строк в сутки, за которыми не видно ни событий, ни ошибок |

Критерий значимости — один на запись и на чтение, `database.SIGNIFICANT_POLL_RUN`:

```sql
error IS NOT NULL OR events_created > 0 OR pushes_sent > 0
```

Отдельный случай — пауза после сбоя. Пока держится `backoff_until`, цикл
повторяется каждые 10 секунд и раньше писал по строке «all accounts backed
off» — под сотню одинаковых записей за четверть часа. Теперь такой цикл
обновляет пульс (в шапке дашборда пробуксовка видна), но в ленту не пишется:
ошибка уже записана тем циклом, который эту паузу и назначил, а текущее
состояние видно в карточке аккаунта.

### Сколько данных держим

`Database.purge_old_data` — скользящее окно в **90 дней** для событий, доставок
и циклов. Не календарный сброс: при сбросе «первого числа» глубина истории
скачет, и поломка 31-го числа разбирается уже по пустой базе. Скользящее окно
всегда даёт одинаковую глубину.

Чистка вызывается **не чаще раза в час** (`PURGE_INTERVAL_SECONDS`). Раньше она
шла в конце каждого цикла: три `DELETE`, которые почти всегда не удаляют
ничего, но каждый берёт write-блокировку SQLite в том же event loop, где
работает опрос.

База, переехавшая с прошлой версии, приносит с собой десятки тысяч пустых
строк — они выметаются разом при первом старте, в `Database._init_schema`.

Дашборд получает три кнопки: **отозвать устройство**, **сбросить аккаунт**
(потребовать повторный вход) и **удалить устройство**. Сегодня отозвать доступ
у телефона нечем.

Отзыв и удаление для телефона одно и то же — `require_device` не находит
`token_hash` и отвечает `401`. Разница в списке устройств: отозванное остаётся
висеть (за ним история доставок), удалённое исчезает и возвращается только
следующим входом. Копить отозванные строки незачем — они мешают смотреть на
живые устройства, поэтому удаление и появилось.
