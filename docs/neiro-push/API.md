...
| Код | Тело | Значение |
|---|---|---|
| `401` | `{"error": "invalid_device_token"}` | Токен неизвестен, отозван или устройство удалено → полный выход |
| `409` | `{"error": "reauth_required"}` | `user_token` протух → повторный вход паролем, аккаунт и пуши живы |
| `429` | `{"error": "rate_limited"}` + `Retry-After` | Больше 60 запросов в минуту с устройства |
| `502` | `{"error": "upstream_error"}` | YClients не ответил или ответил 5xx |
| `504` | `{"error": "upstream_timeout"}` | YClients не уложился в 30 секунд |
