...
Пустой `fcm_token` при входе рабочий токен не затирает (01.09.2026): вход с
телефона, где Firebase не успел ответить, иначе отключал бы пуши на устройстве,
где они работали. Само приложение о таком состоянии больше не молчит — экран
«Уведомления о занятиях» показывает, что мгновенная доставка не поднялась, с
причиной от Firebase и кнопкой «Проверить снова».
