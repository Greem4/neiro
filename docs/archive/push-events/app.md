...
> **Архив.** Документ описывает клиент второго поколения (`neiro-push-events`,
> маршрут `/v2`): регистрацию устройства с токенами YClients в теле и пути
> событий с `device_id`. С этапов 6–8 перехода на `neiro-push` приложение
> работает иначе — актуальное устройство клиента в
> [docs/neiro-push/ARCHITECTURE.md](../../neiro-push/ARCHITECTURE.md).
> Разбор событий, дедуп и применение к календарю с тех пор не менялись — эта
> часть документа по-прежнему верна.
...
| Где | Когда |
|---|---|
| `PushKeepAliveWorker` | каждый тик, **после** `registerNow` (см. ниже) |
...
**Догон идёт после регистрации, а не до** (изменено 27.07.2026 по живым логам
сервиса). Изначально здесь стояло «до `registerNow`» с обоснованием «показать
пропущенные события раньше, чем обновится FCM-токен». На практике это дало
`GET /v1/devices/{id}/events → 404` при первом открытии: сервер про устройство
ещё не знает. Выигрыш прежнего порядка — показ на один HTTP-запрос раньше — не
стоит того, тем более что стартовый курсор приходит именно из ответа регистрации
(§6.4): при старом порядке догон читал курсор `0` и, не отвечай сервер 404,
вывалил бы всю ленту, которую §6.4 как раз и отсекает.

В `LiveApiCoordinator.onStart` порядка строк было мало: `PushRegistrar.onAppForeground`
уходит в собственную корутину и возвращается сразу, поэтому догон стартовал
параллельно с регистрацией. Добавлен suspend-вариант `onAppForegroundNow`,
которого догон дожидается.
