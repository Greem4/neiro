...
> **Архив.** Журнал второго поколения (`neiro-push-events`). Работы третьего
> поколения — [docs/neiro-push/TASKS.md](../../neiro-push/TASKS.md).
...
**27.07.2026 сервис задеплоен на Pi и работает на живых данных** — события
доходят до телефона, доставка `1/1` ([камень 12](#12-первое-живое-подтверждение-доставки)).
Осталось: пересобрать приложение после `44dd8b0` и выждать неделю до Этапа 10.
...
- **Этап 9** — документация по факту (§9 плана): README сервиса, операционный
  документ с troubleshooting, `push-setup.md` помечен как документ старого
  сервиса. Расхождения с планом собраны в [push-events.md §9](../push-events.md#9-расхождения-с-планом).

**27.07.2026 задеплоено на Pi** через новый `scripts/deploy-no-caddy.sh`
(копия штатного `deploy.sh` без шага патча/рестарта общего Caddy — см.
«Подводный камень 9» ниже). `docker compose up -d --build`, `/health`
отвечает `200` и локально на Pi, и публично через `/v2`. `accounts: 0,
devices: 0` — это норма: аккаунт с токенами YClients создаётся сервером
автоматически через `POST /v1/devices/register`, когда приложение впервые
регистрируется; ни один телефон с версией 0.7.0.0 ещё не подключался.
...
| — | Деплой на Pi: `rsync` + `docker compose up -d --build`, без правки Caddy. Проверено: `/health` (локально и `/v2` публично) отвечает `200`, `fcm_configured: true` | `scripts/deploy-no-caddy.sh`, добавлен в git в `e4d1d30` |
| — | Вход в дашборд под префиксом `/v2` ([подводный камень 10](#10-вход-в-дашборд-ломался-под-префиксом-v2)): форма без абсолютного `action`, логин отдаёт страницу вместо `Location`-редиректа, POST принимается и на `/dashboard`. 3 теста | `7b19df2` |
| — | Разделитель версии в текстовом дашборде — `0.7.0.0-debug` слипалось со словом «видели» | `4a260e1` |
| — | Догон перенесён **после** регистрации ([app.md §6.3](app.md#63-точки-вызова)); в `LiveApiCoordinator` для этого добавлен suspend-`onAppForegroundNow` — прежний `onAppForeground` уходил в свою корутину, и догон обгонял регистрацию | `44dd8b0` |
| — | Время дашборда — МСК вместо UTC ([подводный камень 11](#11-дашборд-показывал-время-в-utc)), плюс пометка «время МСК» в шапке и разделители колонок. 1 тест | `7fa2931` |
| — | Аптайм в плитке — сутки строкой сверху, часы `ЧЧ:ММ:СС` снизу ([подводный камень 13](#13-словесный-аптайм-двигал-плитки-шапки)); значения плиток прижаты к низу, чтобы стоять на одной линии. 1 тест | — |
...
- старый `neiro-push` (0.6.9.0) выключен 02.08.2026 (Этап 10);
...
### 1. `backup.sh` никогда не бэкапил `../../neiro-push-events/.env`
...
### 9. `deploy.sh` патчит Caddy, который по факту не участвует в маршруте

При деплое 27.07.2026 обнаружено: штатный `scripts/deploy.sh`, если на Pi
существует `~/server/caddy/Caddyfile` (а он существует), всегда запускает
`patch-pi-caddy-v2.py` и перезапускает **общий** Caddy-контейнер, который
обслуживает старый рабочий `neiro-push` и другие проекты на этом же Pi
(например `medicine.greemlab.ru`). Но по [подводному камню 2](#2-реальная-топология-сети-разошлась-с-планом)
маршрут `/v2` реально идёт через VPS nginx + SSH-туннель, Caddy в этом пути
не участвует вообще — `patch-pi-caddy-v2.py` явно помечен в README как
«написан, но не используется». Деплой сделан временным
`scripts/deploy-no-caddy.sh` (копия `deploy.sh` без этого шага, в git не
добавлен). `deploy.sh` стоит поправить — убрать мёртвый Caddy-шаг, чтобы
не трогать общий рабочий Caddy на каждом деплое; отдельная задача, не
сделана автоматически по правилу «не ломать то, что работает».

### 10. Вход в дашборд ломался под префиксом `/v2`

Найдено 27.07.2026 сразу после деплоя, при попытке зайти в дашборд браузером.
Приложение не знает, что публично оно смонтировано под `/v2` (nginx на VPS
срезает префикс, [камень 2](#2-реальная-топология-сети-разошлась-с-планом)),
поэтому любой абсолютный путь в его ответах уводил браузер мимо сервиса:

- форма логина шла на `action="/dashboard/login"` — браузер со страницы
  `/v2/dashboard` слал POST на `/dashboard/login` **без** `/v2`, а этот путь
  не подпадает под `location /v2/` и уходил в старый `neiro-push` → `404`;
- даже при обходе первого — успешный логин отвечал `303 Location: /dashboard`,
  тоже без `/v2`, то есть снова `404`.

Проверено curl'ом на живом сервисе: `/dashboard` без префикса отдаёт
`404 {"detail":"Not Found"}` с заголовком `Via: 1.1 Caddy` (то есть ответил
старый сервис), с префиксом — `200`.

Починено (`7b19df2`): у формы убран `action` — она уходит на текущий URL,
каким бы префиксом он ни был; логин отдаёт готовую страницу дашборда сразу,
без `Location`-редиректа; POST принимается и на `/dashboard`, и на
`/dashboard/login` (второй — для curl и совместимости с README). Редирект
возвращать нельзя: `meta refresh` на странице перезапросил бы URL логина
методом GET, а такого маршрута нет.

### 11. Дашборд показывал время в UTC
...
### 12. Первое живое подтверждение доставки

27.07.2026, сразу после правок: в ленте семь событий (`CANCELLED` и шесть
`DELETED` по занятию 27.07 18:00) — все с доставкой **1/1**, ошибок опроса нет.
То есть цепочка YClients → дифф → журнал → FCM отработала на живых данных
целиком. Осталось подтверждение с самого телефона, что уведомления видны.

### 13. Словесный аптайм двигал плитки шапки

Замечено пользователем 28.07.2026 по скриншоту дашборда: значения в плитках
стоят на разной высоте. Причина — аптайм словами: «12 часов 16 минут» не влезает
в плитку шириной 150 px, переносится на вторую строку и поднимает высоту всей
строки грида, а соседние плитки при этом остаются с однострочным значением. На
вторые сутки строка меняет длину («2 дня 4 часа»), и ряд прыгает снова — а шапка
...
чтобы HTML и `dash.sh` не разъехались в формулировках.
...
6. ~~**Этап 9** — документация по факту~~ — сделано: README сервиса,
   [docs/push-events.md](../push-events.md), [docs/push-setup.md](../push-setup.md).
7. ~~**Деплой на Pi**~~ — сделано 27.07.2026, `scripts/deploy-no-caddy.sh`.
   `DELETE FROM record_states;` не потребовался — БД была пустой (контейнер
   до этого крутил только каркас Этапа 1 без схемы). Переход на новый push
   обратной совместимости со старым сервисом не имеет — устройства
   переводятся по одному, вместе с обновлением приложения до 0.7.0.0.
8. ~~**Собрать и поставить приложение 0.7.0.0**~~ — сделано 27.07.2026.
   Устройство `KB2003` (версия `0.7.0.0-debug`) зарегистрировалось само при
   первом открытии: в дашборде `company=520135 staff=3618433`, устройств 1,
   курсор 0. Поллер опрашивает YClients каждые 10 с без ошибок.
   События пошли и доставлены `1/1` — см.
   [подводный камень 12](#12-первое-живое-подтверждение-доставки).
9. **Пересобрать приложение** после `44dd8b0` (порядок регистрация → догон):
   правка в коде есть, но на телефоне стоит сборка до неё.
10. **Почистить `deploy.sh`** от мёртвого шага с Caddy (см.
    [подводный камень 9](#9-deploysh-патчит-caddy-который-по-факту-не-участвует-в-маршруте))
    — отдельная задача.
11. ~~**Этап 10** — гашение старого `neiro-push`~~ — сделано 02.08.2026:
    `docker compose down` на Pi (`~/neiro-push`), `restart: unless-stopped`
    контейнер больше не поднимает. Код, `.env` и `data/` на диске не тронуты —
    оставлены для истории/отката. По решению пользователя выключено раньше
    планового «недели стабильной работы», без ожидания.
