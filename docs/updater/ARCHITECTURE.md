...
| `last_known_version_code` | Что видели в прошлый раз (кэш для офлайна и троттлинга) |
| `available_update` | Найденное обновление целиком (JSON, `UpdateOffer`) — предложение, которое поднимает экран при открытии |
...
первая строка описания релиза. Нажатие открывает экран «О программе», и экран
открывается **готовым**: карточка «Доступна 0.2.0» с кнопкой «Обновить» уже
стоит там, второй раз проверять обновления не надо.

Держится это на `available_update`: найденное предложение записывается в
`UpdateCheckCoordinator.checkNow` целиком и переживает закрытие приложения, а
`UpdateViewModel` поднимает его при создании. Прокидывать версию через
`Intent` было бы дешевле, но починило бы только путь из уведомления — экран,
открытый из настроек или без сети, остался бы пустым.
...
Карточка «Доступна 0.2.0» появляется не только после нажатия «Проверить
обновления»: при открытии экрана `UpdateViewModel` поднимает сохранённое
предложение (`available_update`) — из уведомления, из настроек, офлайн, всё
равно. Скачанный, но не поставленный APK при этом важнее: если он есть, экран
показывает «Установить», а не предлагает качать те же 15 МБ заново.
...
- политика уведомлений: та же версия дважды, пропущенная версия, следующая
  после пропущенной;
- сохранённое предложение (`UpdateOffer`): круг через строку без потерь, мусор
  вместо JSON, запись без ссылки на APK или на файл сумм, а также когда
  предложение обязано замолчать — обновились (в том числе мимо приложения) и
  «Пропустить».
