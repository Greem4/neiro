...
Отдельного случая заслуживает PR, который **сам** поднимает версию в
`version.properties`: workflow это видит и второй раз версию не поднимает, а
тегирует ту, что в файле. Ручной путь из-за автоматики не сломался.

Сравнивается при этом **само значение** `VERSION` — в `main` до слияния и
после него, а не факт того, что файл попал в дифф PR. Правка пробела или
комментария в `version.properties` версией не считается: иначе workflow
тегировал бы уже выпущенную версию и падал бы на проверке «тега ещё нет»
(так сгорел выпуск после PR #42).
